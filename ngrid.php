...
function Show_Panel() {
	...
	while ($row = mysql_fetch_assoc($sql)) {
		$link="ngrid.php?mode=panelist&id=".$row['PanelistID'];
...
function Show_Panelist(){
	...
	$sql=mysql_query($query) or die('Query failed: ' . mysql_error());
	while ($row = mysql_fetch_assoc($sql)) {
		$link='ngrid.php?mode=panel&id='.$row['PanelID'];
...
function List_Panelists() {
	...
	$query="Select * from CPDB_PanelistCon as C
			...
			where C.ConID = '".$CFG['ConID']."' order by PanelistLastName";
	$sql=mysql_query($query) or die('Query failed: ' . mysql_error());
#	print $query."<br>";
	while ($row = mysql_fetch_assoc($sql)) {
		$displayname=$row['PanelistPubName'];
...
function my_header(){
	global $CFG;
	$CFG['Duration']=4;
	print "<body bgcolor='#c3c3c3'><table width=475><tr><td><a href='ngrid.php?mode=participants'>List Panelists</a></td>";
	...
	for ($i=1;$i<=$row['ConDays'];$i++){
		...
		for ($h=$starthour;$h<$endhour;$h=$h+$CFG['Duration']){
			...
			print"<td>&nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; </td><td>";
			print "<a href='ngrid.php?mode=grid&start=".date("Y-m-d H:i:s",$blockstart)."&dur=".$CFG['Duration']."'>";
...
#				print "<td colspan=".$ctr.">".$grid[$i][$row['RoomID']]."</td>";
				...
				<a href='ngrid.php?mode=panel&id=".$grid[$i][$row['RoomID']]."'>
